 ...
 *
 * QuickBoot init sequences for Modalix — INDEPENDENT from cold-boot.
 *
 */
...
/* Shared (read-only) cold-boot sequence arrays. */
extern init_element_t seq1_init_ddr_modalix[];
extern init_element_t seq2_init_ddr_modalix[];
extern init_element_t seq4_init_ddr_modalix[];

/* Shared cold-boot sequences table — read only to inherit .size and
 * unique_vals[] populations for the slots we don't override (PRERESET,
 * POSTRESET, FINAL, DIAGNOSTIC, etc.). */
extern init_sequence_t sequences_init_ddr_modalix[PHY_INIT_NUM];

static init_element_t qb_seq3_2D_init_ddr_modalix[] = {
    //Start Qboot
    //start merest seq
    { .type=PHY_INIT_TYPE_PHY, .addr=0xD0000, .value=0x0 }, //DWC_DDRPHYA_APBONLY0__MicroContMuxSel
    { .type=PHY_INIT_TYPE_PHY, .addr=0xd0000, .value=0x0 }, // DWC_DDRPHYA_APBONLY0__MicroContMuxSel
    { .type=PHY_INIT_TYPE_PHY, .addr=0x300A2, .value=0xF00 }, // DWC_DDRPHYA_ACx__AsyncAcTxEn
    { .type=PHY_INIT_TYPE_PHY, .addr=0x30042, .value=0xF0F }, // DWC_DDRPHYA_ACx__TxImpedanceDIFF0T
    { .type=PHY_INIT_TYPE_PHY, .addr=0x30043, .value=0xF0F }, // DWC_DDRPHYA_ACx__TxImpedanceDIFF0C
    { .type=PHY_INIT_TYPE_PHY, .addr=0x300A3, .value=0x800 }, // DWC_DDRPHYA_ACx__AsyncAcTxData
    //{ .type=PHY_INIT_TYPE_PHY, .addr=0x300AC, .value=0x7FF }, // DWC_DDRPHYA_ACx__AcLnDisable
    //{ .type=PHY_INIT_TYPE_PHY, .addr=0x1000F, .value=0x6 }, // DWC_DDRPHYA_DBYTEx__RxReplicaCtl04_p0
    //{ .type=PHY_INIT_TYPE_PHY, .addr=0x200F8, .value=0x0 }, // DWC_DDRPHYA_MASTER0__TxRdPtrInit
    //Wait 30 DFI clk cycles
    { .type=PHY_INIT_TYPE_DELAY, .addr=0x0, .value=0x25 },// delay
    //end of dummy read
    { .type=PHY_INIT_TYPE_PHY, .addr=0x20090, .value=0x1 }, //DWC_DDRPHYA_MASTER0__PorControl
    { .type=PHY_INIT_TYPE_PHY, .addr=0x20060, .value=0x3 }, //DWC_DDRPHYA_MASTER0__MemResetL
    { .type=PHY_INIT_TYPE_PHY, .addr=0x20060, .value=0x2 }, //DWC_DDRPHYA_MASTER0__MemResetL
    { .type=PHY_INIT_TYPE_PHY, .addr=0x20060, .value=0x3 }, //DWC_DDRPHYA_MASTER0__MemResetL
    
    // Wait for TINIT3 - 2ms

    { .type=PHY_INIT_TYPE_DELAY, .addr=0x0, .value=0x7d0  }, //2ms delay
    //end merest seq
    // Full Speed PMU Clock Setting
    { .type=PHY_INIT_TYPE_PHY, .addr=0xd0000, .value=0x0 }, // DWC_DDRPHYA_APBONLY0__MicroContMuxSel
    { .type=PHY_INIT_TYPE_PHY, .addr=0xd0000, .value=0x0 }, // DWC_DDRPHYA_APBONLY0__MicroContMuxSel
    { .type=PHY_INIT_TYPE_PHY, .addr=0xc0080, .value=0x7 }, // DWC_DDRPHYA_DRTUB0_UcclkHclkEnables

    //APB Only register setting
    { .type=PHY_INIT_TYPE_PHY, .addr=0xd0036, .value=0x1 }, // DWC_DDRPHYA_APBONLY0_NeverGateAcCsrClock
    { .type=PHY_INIT_TYPE_PHY, .addr=0xd0037, .value=0x18a }, // DWC_DDRPHYA_APBONLY0_DfiCfgRdDataValidTicks

    /*
     * AN §3.2.3 SRAM ECC Initialization — disable ECC before IMEM/DMEM
     * load so partial-fill of ICC/DCC SRAM cannot trigger ECC errors
     * during firmware execution. Cold seq3_2D writes 0xc0086=0 (ECC
     * enabled) and runs full training which initializes all SRAM; QB
     * skips training and only writes the IMEM/DMEM ranges, so without
     * this the upper SRAM cells have undefined ECC bits.
     * Qboot_debug_4 misses this — the AN does NOT.
     */
    { .type=PHY_INIT_TYPE_PHY, .addr=0xc0086, .value=0x1 }, // DWC_DDRPHYA_DRTUB0_ArcPmuEccCtl (disable for QB)

    //load ICCM DCCM start
    { .type=PHY_INIT_TYPE_FIRMWARE, .addr=0x50000, .value=0x0 }, // ICCM
    { .type=PHY_INIT_TYPE_FIRMWARE, .addr=0x58000, .value=0x1 }, // DCCM
    //Run
    { .type=PHY_INIT_TYPE_PHY, .addr=0xd0000, .value=0x0 }, // DWC_DDRPHYA_APBONLY0_MicroContMuxSel
    { .type=PHY_INIT_TYPE_PHY, .addr=0xd0000, .value=0x0 }, // DWC_DDRPHYA_APBONLY0_MicroContMuxSel
    { .type=PHY_INIT_TYPE_PHY, .addr=0xd0031, .value=0x1 }, //DWC_DDRPHYA_APBONLY0_DctWriteProt
...
//ACSM
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41a2b, .value=0xb00 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41a53, .value=0x2b00 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41a57, .value=0x800 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41a7b, .value=0xb00 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41aa3, .value=0x2b00 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41aa7, .value=0x800 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41b33, .value=0x7b00 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41b5b, .value=0x2b00 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41b5f, .value=0x800 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41bfb, .value=0x7b00 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41c23, .value=0x2b00 }, // 
    ...
    { .type=PHY_INIT_TYPE_PHY_FREQ, .addr=0x41c27, .value=0x800 }, // 
    ...
    //end of seq from training
    //prepare PHY for HW control
    { .type=PHY_INIT_TYPE_PHY, .addr=0x000D00E7, .value=0x600 },
    { .type=PHY_INIT_TYPE_PHY, .addr=0x000C0080, .value=0x2 },
    { .type=PHY_INIT_TYPE_PHY, .addr=0x000D0000, .value=0x1 },
    { .type=PHY_INIT_TYPE_PHY, .addr=0x000D0000, .value=0x1 },//MicroContMuxSel
    { .type=PHY_INIT_TYPE_PHY, .addr=0x000D0000, .value=0x1 },//MicroContMuxSel

};

/*
 * QB sequences table — inherits PRERESET / POSTRESET / FINAL slots from
 * the cold-boot table (read-only sharing — those arrays are never
 * mutated) and overrides 2D_TRAINING with our QB-specific seq3_2D.
 */
static init_sequence_t qb_sequences_init_ddr_modalix[PHY_INIT_NUM];

int32_t get_sequence_init_ddr_modalix_qb(init_sequence_t **sequences)
{
	int i;
	...
	for (i = 0; i < PHY_INIT_NUM; i++)
		qb_sequences_init_ddr_modalix[i] = sequences_init_ddr_modalix[i];

	/* Replace 2D_TRAINING with our QB-specific sequence; all other slots
	 * inherit from cold-boot. */
	qb_sequences_init_ddr_modalix[PHY_INIT_DDR_2D_TRAINING].elements =
		&qb_seq3_2D_init_ddr_modalix[0];
	qb_sequences_init_ddr_modalix[PHY_INIT_DDR_2D_TRAINING].size =
		ARRAY_SIZE(qb_seq3_2D_init_ddr_modalix);

	*sequences = &qb_sequences_init_ddr_modalix[0];
	return ARRAY_SIZE(qb_sequences_init_ddr_modalix);
}
