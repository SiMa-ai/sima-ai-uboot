...
static int xgmac_set_xgmii_speed_10000(struct udevice *dev)
{
	struct xgmac_priv *xgmac = dev_get_priv(dev);
	u32 val;
...
static int xgmac_adjust_link(struct udevice *dev, struct link_state state)
{
	...
	case SPEED_10000:
		en_calibration = true;
		ret = xgmac_set_xgmii_speed_10000(dev);
		break;
	case SPEED_1000:
		en_calibration = true;
...
static int xgmac_start(struct udevice *dev)
{
	...
	if (xgmac->config->ops->xgmac_get_link_state) {
		ret = xgmac->config->ops->xgmac_get_link_state(dev, &state);
		if (ret < 0) {
			pr_err("Failed to get link state: %d\n", ret);
			goto err_shutdown_phy;
		}
	}

	if (!xgmac->phy->link && !state.link) {
		pr_err("No link\n");
		goto err_shutdown_phy;
	}

	ret = xgmac_adjust_link(dev, state);
	if (ret < 0) {
...
#ifdef CONFIG_DWC_ETH_XGMAC_SOCFPGA
	{
		.compatible = "intel,socfpga-dwxgmac",
		.data = (ulong)&xgmac_socfpga_config
	},
#endif
#ifdef CONFIG_DWC_ETH_XGMAC_MODALIX
	{
		.compatible = "simaai,modalix-dwxgmac",
		.data = (ulong)&xgmac_modalix_config
	},
#endif
	{ }
};
